...
from bot.states.beats import AddBeat, BeatFilter
...
async def moderate_work(call: CallbackQuery, session: AsyncSession, bot: Bot):
    ...
    if action == "approve":
        ...
        notify = t("beat_approved_notify", author.lang, title=work.title)
    else:
        work.moderation_status = ModerationStatus.rejected
        admin_msg = t("mod_rejected_admin", Lang.ru)
        notify = t("beat_rejected_notify", author.lang, title=work.title)
    await session.commit()
...
async def open_catalog(message: Message, state: FSMContext, session: AsyncSession, user: User):
    """Точка входа из меню «Готовые биты»."""
    category = await repo.get_category_by_code(session, READY_BEATS)
    if category is None:
...
async def filter_setup(call: CallbackQuery, state: FSMContext, session: AsyncSession, user: User):
    ...
    await call.message.edit_text(
        t("filter_genre", user.lang), reply_markup=genre_keyboard(user.lang, genres)
    )
    await call.answer()


@router.callback_query(BeatFilter.genre, F.data.startswith("fltgenre:"))
async def filter_pick_genre(call: CallbackQuery, state: FSMContext, user: User):
...
async def _render_caption(session: AsyncSession, work_id: int, pos: int, total: int, lang: Lang):
...
async def _start_carousel(event, state: FSMContext, session: AsyncSession, user: User, ids: list[int]):
    ...
    work, caption = await _render_caption(session, ids[0], 1, len(ids), user.lang)
    target = event.message if isinstance(event, CallbackQuery) else event
    await target.answer_photo(
        work.cover_file_id,
        caption=caption,
        reply_markup=beat_card_keyboard(user.lang, work.id),
    )
...
async def carousel_nav(call: CallbackQuery, state: FSMContext, session: AsyncSession, user: User):
    ...
    work, caption = await _render_caption(session, ids[idx], idx + 1, len(ids), user.lang)
